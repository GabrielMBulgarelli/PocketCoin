...
export function SettingsView({ settings, loading, loadError }: { settings?: Settings; loading: boolean; loadError: boolean }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[2fr_1fr]"><DashboardCard title="Display preferences" description="Saved in your local PocketCoin database" actions={<button form="display-preferences-form" disabled={mutation.isPending} className="min-h-11 rounded-lg bg-primary px-4 text-sm font-medium text-primary-foreground disabled:opacity-50" type="submit">{mutation.isPending ? "Saving…" : "Save settings"}</button>}><form id="display-preferences-form" className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); setMessage(""); mutation.mutate(form); }}>
    <label className="grid gap-1 text-sm font-medium">Currency<select className={control} value={form.base_currency} onChange={(event) => setForm({ ...form, base_currency: event.target.value })}><option value="CRC">CRC</option><option value="USD">USD</option><option value="EUR">EUR</option></select></label>
    <label className="grid gap-1 text-sm font-medium">Locale<select className={control} value={form.locale} onChange={(event) => setForm({ ...form, locale: event.target.value })}><option value="es-CR">es-CR</option><option value="en-US">en-US</option></select></label>
    ...
    {(mutation.isError || message) && <div className="sm:col-span-2">{mutation.isError && <p className="text-sm text-destructive" role="alert">{mutation.error.message}</p>}{message && <p className="text-sm text-muted-foreground" role="status">{message}</p>}</div>}
  </form></DashboardCard><div className="grid self-start gap-5"><DashboardCard title="Import CSV" description="Add reviewed transactions from a bank statement"><p className="mb-4 text-sm text-muted-foreground">Upload a CSV, map its columns, and review every row before adding transactions.</p><Button variant="outline" asChild><a href="#/import">Import CSV</a></Button></DashboardCard><DashboardCard title="Full transaction export" description="Every ledger row in a spreadsheet-safe CSV"><p className="mb-4 text-sm text-muted-foreground">The export uses {form.base_currency.toUpperCase()} and includes all dates, accounts, categories, tags, and transfers.</p><button className="min-h-11 rounded-lg border bg-background px-4 text-sm font-medium disabled:cursor-not-allowed disabled:opacity-50" type="button" disabled={csvExport.status === "exporting"} onClick={() => void csvExport.start(() => exportTransactions())}>{csvExport.status === "exporting" ? "Exporting…" : "Export full history"}</button>{csvExport.message && <div className="mt-3"><ExportStatusAlert status={csvExport.status} message={csvExport.message} /></div>}</DashboardCard></div><div className="xl:col-span-2"><DataSafetyCard locale={form.locale} /></div></div>;
}
