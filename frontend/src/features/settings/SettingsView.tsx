...
export function SettingsView({ settings, loading, loadError }: { settings?: Settings; loading: boolean; loadError: boolean }) {
  ...
  </form></DashboardCard><DashboardCard title="Full transaction export" description="Every ledger row in a spreadsheet-safe CSV"><p className="mb-4 text-sm text-muted-foreground">The export uses {form.base_currency.toUpperCase()} and includes all dates, accounts, categories, tags, and transfers.</p><button className="min-h-11 rounded-lg border bg-background px-4 text-sm font-medium disabled:cursor-not-allowed disabled:opacity-50" type="button" disabled={csvExport.status === "exporting"} onClick={() => void csvExport.start(() => exportTransactions())}>{csvExport.status === "exporting" ? "Exporting…" : "Export full history"}</button>{csvExport.message && <div className="mt-3"><ExportStatusAlert status={csvExport.status} message={csvExport.message} /></div>}</DashboardCard><div className="xl:col-span-2"><DataSafetyCard locale={form.locale} /></div></div>;
}
