...
export function QuickAddDialog({ open, onOpenChange, onCreated }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return createTransaction({ ...shared, kind: mode, financial_account_id: values.accountId ? Number(values.accountId) : null, category_id: Number(values.categoryId), tag_ids: values.tagId ? [Number(values.tagId)] : [], recurrence: values.repeats ? { frequency: values.frequency, end_date: values.endMode === "date" ? values.endDate : null, is_debt_payment: mode === "expense" && values.isDebt } : undefined });
    },
    onSuccess: async () => {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>Quick add</DialogTitle><DialogDescription>Record money movement without leaving your current workspace.</DialogDescription></DialogHeader>
    ...
    {referencePending ? <p className="py-8 text-center text-sm text-muted-foreground" role="status">Loading accounts and categories…</p> : referenceError ? <p className="py-8 text-center text-sm text-destructive" role="alert">Reference data could not be loaded. Close and try again.</p> : <form onSubmit={submit}><div className="grid gap-4 sm:grid-cols-2">
      ...
      {mode !== "transfer" && <><label className="flex min-h-11 items-center gap-2 text-sm font-medium sm:col-span-2"><input type="checkbox" checked={values.repeats} onChange={(event) => update("repeats", event.target.checked)} /> Repeat this transaction</label>{values.repeats && <><label className="text-sm font-medium">Frequency<select aria-label="Frequency" className={control} value={values.frequency} onChange={(event) => update("frequency", event.target.value as Values["frequency"])}><option value="weekly">Weekly</option><option value="monthly">Monthly</option><option value="yearly">Yearly</option></select></label><label className="text-sm font-medium">Ends<select aria-label="Ends" className={control} value={values.endMode} onChange={(event) => update("endMode", event.target.value as Values["endMode"])}><option value="never">Never</option><option value="date">On date</option></select></label>{values.endMode === "date" && <label className="text-sm font-medium sm:col-span-2">End date<input aria-label="End date" className={control} min={values.date} onChange={(event) => update("endDate", event.target.value)} required type="date" value={values.endDate} /></label>}{mode === "expense" && <label className="flex min-h-11 items-center gap-2 text-sm font-medium sm:col-span-2"><input type="checkbox" checked={values.isDebt} onChange={(event) => update("isDebt", event.target.checked)} /> Recurring debt payment</label>}</>}</>}
