    ...
    <div className="relative min-w-0">
      ...
      <div className={`space-y-5 transition-opacity ${isUpdating ? "opacity-70" : ""}`}>
        <div className="grid gap-5 xl:grid-cols-2">
          ...
          {mode === "cash-flow" && (comparison.isPending || comparison.isError ? <SectionState title="Period comparison" period={period} query={comparison} /> : <PeriodComparisonCard data={comparison.data} context={`${metric.replace("_", " ")} vs previous period and prior year`} emptyLabel="No comparable period yet." formatMinor={money} shortDate={shortDate} period={<PeriodLabel kind="range" startDate={filters.start_date} endDate={filters.end_date} locale={locale} />} actions={<MetricSelector value={metric} onChange={setMetric} />} />)}
