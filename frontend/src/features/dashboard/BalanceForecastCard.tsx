...
export function BalanceForecastCard({ forecast, formatMinor, shortDate }: Props) {
  ...
  return <section className="rounded-xl border bg-card p-5 shadow-sm xl:col-span-2">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold tracking-tight">Balance forecast</h2><p className="mt-1 text-xs text-muted-foreground">Estimate · {shortDate(forecast.forecast_start)} to {shortDate(forecast.forecast_end)}</p></div><div className="rounded-lg bg-muted px-3 py-2 text-right"><p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Ending estimate</p><p className="text-xl font-semibold tabular-nums">{formatMinor(forecast.ending_balance_minor)}</p></div></div>
    <div className="mt-5 h-72" aria-label="Estimated balance components"><ResponsiveContainer width="100%" height="100%"><BarChart data={rows} layout="vertical" margin={{ left: 8, right: 12 }}><CartesianGrid horizontal={false} strokeDasharray="3 3" /><XAxis type="number" tickFormatter={(value) => formatMinor(Number(value))} /><YAxis dataKey="label" type="category" width={104} tick={{ fontSize: 12 }} /><Tooltip formatter={(value) => formatMinor(Number(value))} /><ReferenceLine x={0} stroke="var(--border)" /><Bar dataKey="value" name="Balance effect" radius={[4, 4, 4, 4]}>{rows.map((row) => <Cell key={row.label} fill={row.value < 0 ? "#f97316" : row.label === "Estimate" ? "var(--primary)" : "#22c55e"} />)}</Bar></BarChart></ResponsiveContainer></div>
