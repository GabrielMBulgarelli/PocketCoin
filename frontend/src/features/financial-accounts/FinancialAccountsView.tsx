...
export function FinancialAccountsView({ currency, locale }: { currency: string; locale: string }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    mutation.mutate({ id: editing?.id ?? null, data: { name: data.get("name"), ...(editing ? { is_active: data.get("is_active") === "on" } : { kind: data.get("kind") }), opening_balance_minor: amount, opening_balance_date: data.get("opening_balance_date"), credit_limit_minor: credit ? Math.round(Number(credit) * 100) : null } }, { onError: (value) => setError(value instanceof Error ? value.message : "The account could not be saved.") });
  };
  ...
  return <section>
    ...
    <Dialog open={editing !== undefined} onOpenChange={(openValue) => { if (!openValue) setEditing(undefined); }}><DialogContent><DialogHeader><DialogTitle>{editing ? "Edit account" : "Add account"}</DialogTitle><DialogDescription>{editing ? "Account type cannot be changed after creation." : "Set the opening point for this account."}</DialogDescription></DialogHeader><form onSubmit={submit}><div className="grid gap-4 sm:grid-cols-2"><label className="sm:col-span-2 text-sm font-medium">Name<input className={field} defaultValue={editing?.name ?? ""} name="name" /></label><label className="text-sm font-medium">Type<select className={field} defaultValue={editing?.kind ?? "checking"} disabled={Boolean(editing)} name="kind"><option value="cash">Cash</option><option value="checking">Checking</option><option value="savings">Savings</option><option value="credit_card">Credit card</option><option value="overdraft">Overdraft</option><option value="loan">Loan</option></select></label><label className="text-sm font-medium">Opening date<input className={field} defaultValue={editing?.opening_balance_date ?? today()} name="opening_balance_date" type="date" /></label><label className="text-sm font-medium">Opening balance<input className={field} defaultValue={editing ? editing.opening_balance_minor / 100 : 0} min="0" name="opening_balance" step="0.01" type="number" /></label><label className="text-sm font-medium">Credit limit (optional)<input className={field} defaultValue={editing?.credit_limit_minor ? editing.credit_limit_minor / 100 : ""} min="0.01" name="credit_limit" step="0.01" type="number" /></label>{editing && <label className="flex items-center gap-2 text-sm font-medium sm:col-span-2"><input defaultChecked={editing.is_active} name="is_active" type="checkbox" /> Active</label>}</div>{(error || mutation.isError) && <p className="mt-4 text-sm text-destructive" role="alert">{error}</p>}<DialogFooter className="mt-6"><Button disabled={mutation.isPending} type="submit">{mutation.isPending ? "Saving…" : "Save account"}</Button></DialogFooter></form></DialogContent></Dialog>
  </section>;
}
