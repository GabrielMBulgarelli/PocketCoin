...
export async function getDashboard(filters: DashboardFilters): Promise<DashboardData> {
  const [summary, forecast, creditUtilization, creditAccounts, recurringDebts, debtToIncome, cashFlow, comparison, categories, structure, recent, upcoming] = await Promise.all([
    ...
    getDashboardEndpoint<CategoryPoint[]>("category-spending", filters), getDashboardEndpoint<CategoryPoint[]>("expense-structure", filters), getDashboardEndpoint<RecentActivity[]>("recent-transactions", filters), getDashboardEndpoint<PlannedPayment[]>("upcoming-payments", filters),
  ]);
